...
public class FridayWeekly implements WeeklyWork {
	...
	public List<MusicInfo> getWeeklyWorkInfo(Object[] object) throws WeeklyMusicInfoException {
		...
		if (!(object[0] instanceof WebDriver) || !(object[1] instanceof String)
			...
			throw new WeeklyMusicInfoException(PARA_MESSAGE);
		...
		try {
			...
			webDriver.findElement(By.id("ui-tabs-" + (musicStyleEnum.ordinal() + 1)))
					...
					.findElement(By.className("topchart_rank_tab_usual")).findElements(By.tagName("li"))
					.get(musicTypeEnum.ordinal()).findElement(By.tagName("a")).click();
			Thread.sleep(5000);

			// 點選顯示更多
			((JavascriptExecutor) webDriver).executeScript("arguments[0].click()",
					webDriver.findElement(By.id("ui-tabs-" + (musicStyleEnum.ordinal() + 1)))
							.findElement(By.className("topchart_right_box")).findElement(By.className("topchart_box"))
			...
			throw new WeeklyMusicInfoException(CLICK_MESSAGE);
		}
